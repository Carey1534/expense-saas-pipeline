...
function Toast({ message, type, visible }: { message: string; type: 'success' | 'error'; visible: boolean }) {
  return (
    <div
      className={`fixed top-4 right-4 z-50 flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg border transition-all duration-300 ${
  ...
  async function handleDecision(decision: 'APPROVED' | 'REJECTED') {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => router.push(backUrl), 1400);
      } else {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <button onClick={() => router.push(backUrl)} className="mt-4 text-blue-600 hover:text-blue-800">
            ← Back to expenses
          </button>
        </div>
      </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            {!canDecide && expense.status === 'APPROVED' && (
              <div className={`mt-8 p-4 rounded-lg flex items-center gap-3 transition-all duration-500 ${
                flashDecision === 'APPROVED'
                  ? 'bg-green-100 border border-green-300 scale-[1.02]'
            ...
            {!canDecide && expense.status === 'REJECTED' && (
              <div className={`mt-8 p-4 rounded-lg flex items-center gap-3 transition-all duration-500 ${
                flashDecision === 'REJECTED'
                  ? 'bg-red-100 border border-red-300 scale-[1.02]'
