...
import { inferCategory, getCategoryByKey, CATEGORIES } from '@/lib/categories';
...
function CategorySelect({ value, onChange, disabled }: {
  ...
  const cat = getCategoryByKey(value);
  return (
    <div>
      <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1 block">Category</label>
      <div className={`flex items-center rounded-lg border-2 transition-colors ${disabled ? 'bg-gray-50 border-gray-200' : 'border-gray-200 bg-white focus-within:border-blue-400'}`}>
        <span className="pl-2.5 text-sm">{cat.emoji}</span>
        <select
          value={value}
          onChange={e => onChange(e.target.value)}
          disabled={disabled}
          className="flex-1 px-2 py-2 text-sm bg-transparent outline-none rounded-lg font-medium text-gray-800 disabled:text-gray-400 disabled:cursor-not-allowed"
        >
...
function VendorField({ value, fieldScore, onChange, disabled, suggestions }: {
  value: string; fieldScore: number; onChange: (v: string) => void;
  disabled?: boolean; suggestions: string[];
}) {
    ...
    <div ref={wrapRef}>
      ...
      <div className="relative">
        <div className={`flex items-center rounded-lg border-2 transition-colors ${
          ...
          />
          {value && !disabled && (
            <button type="button" onClick={() => onChange('')} className="pr-2 text-gray-300 hover:text-gray-500 transition-colors flex-shrink-0">
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
        <div className={`h-full rounded-full transition-all ${score >= 0.85 ? 'bg-green-500' : score >= 0.65 ? 'bg-yellow-400' : 'bg-red-400'}`}
          style={{ width: `${pct}%` }} />
      </div>
      <span className={`text-xs font-bold w-9 text-right ${c.text}`}>{pct}%</span>
      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${c.bg} ${c.text} border ${c.border}`}>{c.label}</span>
    </div>
  );
}
...
function ReceiptPreview({ expense, expanded, onToggle, signedSrc }: {
  expense: Expense; expanded: boolean; onToggle: () => void; signedSrc?: string;
}) {
...
function ReviewCard({
  item, index, onFieldChange, onDecision, onSaveAndApprove, vendorSuggestions, signedSrc,
}: {
  item: ReviewItem; index: number;
  ...
  return (
    <div className={`bg-white rounded-xl border-2 shadow-sm transition-all ${
      ...
      {/* Card header */}
      <div className={`px-4 py-3 rounded-t-xl border-b flex items-center justify-between gap-3 ${
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-gray-500">${expense.total_amount?.toFixed(2)} · {expense.expense_date ?? '—'}</p>
          </div>
        </div>
      ...
      <div className="p-4">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-1">
            ...
            <ReceiptPreview expense={expense} expanded={receiptExpanded} onToggle={() => setReceiptExpanded(v => !v)} signedSrc={signedSrc} />
            {signedSrc && (
          ...
          <div className="lg:col-span-2 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <VendorField value={draft.vendor_name} fieldScore={vendorConf}
                onChange={v => onFieldChange(expense.id, 'vendor_name', v)}
                disabled={disabled} suggestions={vendorSuggestions} />
              <EditableField label="Date" value={draft.expense_date} fieldScore={dateConf}
                type="date" onChange={v => onFieldChange(expense.id, 'expense_date', v)} disabled={disabled} />
              <EditableField label="Total Amount" value={draft.total_amount} fieldScore={totalConf}
                type="number" prefix="$" onChange={v => onFieldChange(expense.id, 'total_amount', v)} disabled={disabled} />
              <EditableField label="Tax Amount" value={draft.tax_amount} fieldScore={taxConf}
                type="number" prefix="$" onChange={v => onFieldChange(expense.id, 'tax_amount', v)} disabled={disabled} />
              <div className="sm:col-span-2">
                <CategorySelect value={draft.category} onChange={v => onFieldChange(expense.id, 'category', v)} disabled={disabled} />
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
              {dirty ? (
                <button onClick={() => onSaveAndApprove(expense.id)} disabled={disabled}
                ...
                <button onClick={() => onDecision(expense.id, 'APPROVED')} disabled={disabled}
                  ...
                  {deciding === 'APPROVED' ? <span className="animate-spin rounded-full h-3.5 w-3.5 border-b-2 border-white" /> : (
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button onClick={() => onDecision(expense.id, 'REJECTED')} disabled={disabled}
                ...
                {deciding === 'REJECTED' ? <span className="animate-spin rounded-full h-3.5 w-3.5 border-b-2 border-red-500" /> : (
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function GlobalReviewQueuePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        // Load all expenses + all clients/projects in parallel
        const [expRes, clientsRes] = await Promise.all([
          fetch('/api/expenses'),
          fetch('/api/clients'),
        ]);
        ...
        if (clientsRes.ok) {
          ...
          const projectFetches = clients.map(c =>
            fetch(`/api/clients/${c.id}/projects`).then(r => r.ok ? r.json() : [])
          );
          const allProjectArrays: { id: string; name: string }[][] = await Promise.all(projectFetches);
          const nameMap: Record<string, string> = {};
          allProjectArrays.flat().forEach(p => { nameMap[p.id] = p.name; });
          setProjectNames(nameMap);
        }

        // Filter to reviewable statuses only
        ...
        setItems(reviewable.map(e => ({
          ...
          draft: {
            ...
            category: (e as Expense & { category?: string }).category ?? inferCategory(e.vendor_name).key,
          },
          dirty: false,
          saving: false,
          deciding: null,
        ...
        // Fetch signed URLs progressively
        reviewable
  ...
  const handleFieldChange = useCallback((id: string, field: keyof ReviewField, value: string) => {
    setItems(prev => prev.map(item => {
      ...
      const origCategory = (orig as Expense & { category?: string }).category ?? inferCategory(orig.vendor_name).key;
      const dirty =
        ...
        newDraft.category !== origCategory;
  ...
  const handleDecision = useCallback(async (id: string, decision: 'APPROVED' | 'REJECTED') => {
    ...
    try {
      const res = await fetch('/api/approve', {
        ...
        body: JSON.stringify({ org_id: item.expense.org_id, expense_id: id, decision }),
      });
      ...
      setItems(prev => prev.map(i => i.expense.id === id ? { ...i, deciding: null, error: e instanceof Error ? e.message : 'Action failed' } : i));
    }
  }, [items]);
    ...
    try {
      const patchRes = await fetch(`/api/expenses/${id}`, {
        ...
        body: JSON.stringify({
          ...
          category: item.draft.category || undefined,
        }),
      });
      if (!patchRes.ok) { const d = await patchRes.json(); throw new Error(d.error || 'Save failed'); }

      const approveRes = await fetch('/api/approve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ org_id: item.expense.org_id, expense_id: id, decision: 'APPROVED' }),
      });
      if (!approveRes.ok) { const d = await approveRes.json(); throw new Error(d.error || 'Approve failed'); }
      ...
      setItems(prev => prev.map(i => i.expense.id === id ? { ...i, saving: false, error: e instanceof Error ? e.message : 'Something went wrong' } : i));
    }
  }, [items]);
  ...
  // Group visible items by project_id for section headers
  const grouped: { projectId: string | null; projectName: string; items: (ReviewItem & { globalIndex: number })[] }[] = [];
  const groupMap = new Map<string, typeof grouped[0]>();
  visibleItems.forEach((item, idx) => {
    const pid = item.expense.project_id ?? '__none__';
    const pname = (item.expense.project_id && projectNames[item.expense.project_id]) || 'Unassigned';
    if (!groupMap.has(pid)) {
      const g = { projectId: item.expense.project_id ?? null, projectName: pname, items: [] as (ReviewItem & { globalIndex: number })[] };
      groupMap.set(pid, g);
      grouped.push(g);
    }
    groupMap.get(pid)!.items.push({ ...item, globalIndex: idx });
  });
          ...
          {[1, 2, 3].map(i => (
            <div key={i} className="bg-white rounded-xl border-2 border-gray-200 p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[0, 1, 2, 3].map(j => <div key={j} className="h-16 bg-gray-100 rounded-lg animate-pulse" />)}
              </div>
            </div>
          ))}
        </div>
    ...
    return (
      <div className="p-6 max-w-4xl">
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">No pending expenses across any project.</p>
    ...
    <>
      ...
      <div className="p-6 max-w-4xl">
        ...
        <div className="mb-6">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">All projects · {totalCount} expense{totalCount !== 1 ? 's' : ''} pending</p>
            </div>
            <div className="flex items-center gap-2">
              {doneCount > 0 && (
        ...
        <div className="mb-5 flex items-start gap-3 p-3.5 bg-blue-50 border border-blue-200 rounded-xl text-sm text-blue-800">
          ...
          <span>Fields are <strong>color-coded by AI confidence</strong>. Edit any field to correct OCR errors, then click <strong>Save &amp; Approve</strong>.</span>
        </div>
            ...
            {grouped.map(group => (
              <div key={group.projectId ?? '__none__'}>
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2 className="text-sm font-bold text-gray-700">{group.projectName}</h2>
                  <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">
                    {group.items.filter(i => !i.done).length} pending
                  </span>
